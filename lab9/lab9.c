...
port number). IP address: 127.0.0.1 (localhost), Port: 8000. Lines 10 and 8
define ADDR as "127.0.0.1" and PORT as 8000
...
data from? How can you tell in the code? The data comes from standard input
(keyboard/terminal input), Line 38 reads from STDIN_FILENO, which means it reads
from standard input

4. How does the client program end? How can you tell that in the code?
   The client ends when EOF is encountered on standard input (when user presses
Ctrl+D or input ends). Line 38 shows the while loop continues while read()
returns > 1. When EOF is reached, read() returns 0 or -1, breaking the loop and
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(client->cfd, buf, BUF_SIZE - 1)) > 0) {
    ...
    if (num_read > 0) {
      ...
      printf("Msg #%4d; Client ID %d: %s\n", current_count, client->client_id,
             buf);
    }
  }
